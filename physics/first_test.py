        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_a:
                    player.surface_velocity = 100, 0
                if event.key == pygame.K_d:
                    player.surface_velocity = -100, 0
                if event.key == pygame.K_w:
                    print("W pressed", well_grounded)
                    if well_grounded:
                        print("jump init")
                        impulse = (0, player.body.mass * (grounding["body"].velocity.y + jump_v))
                        player.body.apply_impulse_at_local_point(impulse)

        if grounding["body"] != None:
            player.friction = 0.005#40/gravity_y
        else:
            player.friction = 0
